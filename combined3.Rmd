...
- Longitudinal data can inform survival outcome by providing time dependent information
  - e.g. Increasing size of ventricles likely provides information about time to AD
...
- This simple change in notion introduces a shift in the modeling framework, as this is now a functional mixed effects model
...
- We will leave the fixed covariates as is, however, we will include the **REs** from the longitudinal model above in place of the time varying covariates
...
- Henderson Et. Al. 2000 provides the a more general form of the joint model, for which this is a subset of
...
- Due to the treatment of high dimensional data, the likelihood here (to be presented), can be expressed in a form similar to that above, but does not have an explicit form
...
- Many approaches use an EM algorithm adapted to the specific forumlation of the joint model
...
- The authors show this can be estimated using Monte Carlo integration
